...
Array
  $ ./demoInterpret.exe << EOF
  > let val f = fn x arr => x::arr in f 1 [2, 3] end
  [1, 2, 3]

Equality types
  $ ./demoInterpret.exe << EOF
  > let val r = fn y1 y2 x1 x2 => y1 = y2 orelse not (x1 = x2) in r 1 2 3 4 end
  true
...
Factorial
  $ ./demoInterpret.exe << EOF
  > let val rec fix = (fn f x => f (fix f) x) 
  > val fact = (fn self n => if n = 0 then 1 else n * self (n - 1)) 
...
Errors
  ...
  $ ./demoInterpret.exe << EOF
  > let val f = fn _ => _ in f true end
